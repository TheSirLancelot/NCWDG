...
/*
 *	The compare function (compare_func) is user implemented.  * It takes in two parameters:
 *	1. void * of value 1  * 	2. void * of value 2
 *
 ...
 *	as a first argument to compare_func and the data field in the
...
int removeHead(list ** listHead) {
	...
	if (newHead == NULL) { //head is the only element
		free(curr->listRoot);
		free(curr); (*listHead) = NULL;
		return 1;
	}
	newHead->prev = NULL;
	(*listHead) = newHead;
	newHead->listRoot->head = newHead;
	//free(oldhead->data);
	free(oldhead);
	return 1;
}
 ...
 *	as the memory allocated to the linked list structure  * is freed.  */
...
clientStruct * createClientStruct(int client_sock, char * clientIPAddr) {
	...
	memcpy(clientStructPTR->ipAddr, clientIPAddr,
			min(strlen(clientIPAddr), IPADDRLEN));
